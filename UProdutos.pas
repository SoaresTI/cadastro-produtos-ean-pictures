...
begin
  if trim(edt_valor_venda.Text) = '' then
  begin
    showmessage('Informe um valor de venda');
  ...
  begin
    showmessage
      ('Este produto não possui imagem, tente usar outro código de barras!');
    Abort;
  end;
...
begin
  ...
  else if RESTRequest1.Response.StatusCode = 404 then
  begin
    showmessage('Não localizamos informações para o produto do codigo ean: ' +
      edt_ean.Text);
    Abort;
  end;
  ...
  begin
    ...
    if DownloadArquivo('http://www.eanpictures.com.br:9000/api/gtin/' +
      edt_ean.Text, 'c:\imagem\' + edt_ean.Text + '.png') then
    begin
      try
        img_produto.Picture.LoadFromFile(imagemPNG);
        CDS_Produtosimagem.AsString := imagemPNG;
      except
        renamefile(imagemPNG, imagemJPG);
        img_produto.Picture.LoadFromFile(imagemJPG);
